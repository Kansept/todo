...
class Paginator
{
    ...
    public function createLinks($links)
    {
        ...
        $html .= '<li class="page-item ' . $class . '"><a href="' . $this->setGetParam(['limit' => $this->limit, 'page' => ($this->page - 1)]) . '" class="page-link">&laquo;</a></li>';
   
        if ($start > 1) {
            $html .= '<li class="page-item"><a href="' . $this->setGetParam(['limit' => $this->limit, 'page' => 1]) . '" class="page-link">1</a></li>';
            $html .= '<li class="disabled"><span>...</span></li>';
        }
   
        for ($i = $start ; $i <= $end; $i++) {
            $class = ($this->page == $i) ? "active" : "";
            $html .= '<li class="page-item ' . $class . '"><a href="' . $this->setGetParam(['limit' => $this->limit, 'page' => $i]) . '" class="page-link">' . $i . '</a></li>';
        }
   
        if ($end < $last) {
            $html .= '<li class="page-item disabled"><span>...</span></li>';
            $html .= '<li class="page-item"><a href="' . $this->setGetParam(['limit' => $this->limit, 'page' => $last]) . '" class="page-link">' . $last . '</a></li>';
        }
   
        $class = ($this->page == $last) ? "disabled" : "";
        $html .= '<li class="page-item ' . $class . '"><a href="' . $this->setGetParam(['limit' => $this->limit, 'page' => ($this->page + 1)]) . '" class="page-link">&raquo;</a></li>';
